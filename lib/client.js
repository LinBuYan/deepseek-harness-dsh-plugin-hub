window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var WATCH_CACHE_TTL = 24 * 60 * 60 * 1000; /* 目录与星数缓存 24 小时 */
		...
		function WatchPanel() {
			...
			var load = React.useCallback(function (force) {
				var now = Date.now();
				if (force && now - lastRef.current < WATCH_REFRESH_THROTTLE && resultState.current) return;
			...
			var doCheckRisk = async function (p) {
				...
				try {
					...
					if (!pkg) {
						setRisk({ phase: "error", error: "npm 上未找到与仓库 " + p.repo + " 对应的包，可能尚未发布到 npm。" });
						return;
					}
			...
			var doInstall = async function (p) {
				...
				if (seq !== opSeqRef.current) return;
				if (!pkg) {
					setInstall({ phase: "error", error: "npm 上未找到与仓库 " + p.repo + " 对应的包，可能尚未发布到 npm。" });
					return;
				}
				setDraftPkg(pkg);
				setInstall({ phase: "confirm", pkg: pkg });
				...
				if (risk.phase === "done" && risk.report) {
					...
					return React.createElement("div", { className: "hub-panel" },
						React.createElement("div", null,
							...
							React.createElement("span", { style: { marginLeft: "6px", fontWeight: 600 } }, "npm 包：" + r.pkg)
						),
			...
			function installView() {
				...
				if (install.phase === "confirm") {
					return React.createElement("div", { className: "hub-panel" },
						React.createElement("div", null, "将执行（web profile，包名可修改）："),
						...
						React.createElement("div", { className: "hub-actions" },
							React.createElement("button", {
								...
								onClick: function () { startInstall(sel, draftPkg.trim()); }
							}, "确认安装"),
							React.createElement("button", { className: "hub-act", onClick: function () { setInstall(null); } }, "取消")
						)
					);
				}
			...
			var card = React.createElement("div", {
				...
				React.createElement("div", {
					...
					React.createElement("span", { className: "hub-title", title: "DSH 插件中心：扫描 + 插件热榜 + 已装插件管理" },
						tab === "watch" ? "dsh 插件热榜" : tab === "installed" ? "已装插件" : "GitHub dsh 扫描 · " + ORG),
					React.createElement("button", {
