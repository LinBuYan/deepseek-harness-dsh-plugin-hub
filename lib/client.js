window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = [
			...
			".hub-body { overflow-y: auto; padding: 10px 12px 12px; margin-right: 10px; display: flex; flex-direction: column; gap: 9px; }",
			...
			".hub-rs-e { right: 0; top: 0; bottom: 0; width: 10px; cursor: ew-resize; }",
			".hub-rs-w { left: 0; top: 0; bottom: 0; width: 10px; cursor: ew-resize; }",
			".hub-rs-n { left: 0; right: 0; top: 0; height: 10px; cursor: ns-resize; }",
			".hub-rs-s { left: 0; right: 0; bottom: 0; height: 10px; cursor: ns-resize; }",
			".hub-rs-ne { right: 0; top: 0; width: 20px; height: 20px; cursor: nesw-resize; }",
			".hub-rs-sw { left: 0; bottom: 0; width: 20px; height: 20px; cursor: nesw-resize; }",
			".hub-rs-nw { left: 0; top: 0; width: 20px; height: 20px; cursor: nwse-resize; }",
			".hub-rs-se { right: 0; bottom: 0; width: 20px; height: 20px; cursor: nwse-resize;",
		 ...
		 * 镜像降级链：raw.githubusercontent（最新）→ jsDelivr（国内 CDN 兜底）→ GitHub API readme
		...
		function parseOverviewSections(auto, manual) {
			var out = { installed: [], skills: [], presets: [], usage: {}, usageText: "", constraints: [] };
			var cur = null;
			function feed(block) {
				...
				for (var i = 0; i < lines.length; i++) {
					...
					if (t === "agent_presets:") { cur = "presets"; continue; }
					if (t === "skills:") { cur = "skills"; continue; }
					...
					if (cur === "presets" && /^-\s+/.test(t)) out.presets.push(t.replace(/^-\s+/, ""));
					if (cur === "skills" && /^-\s+/.test(t)) out.skills.push(t.replace(/^-\s+/, ""));
		...
		function buildAdvice(plugin, ov, risk) {
			...
			/* 多信号匹配：分类关键词 / 插件名·描述 / 业务能力词库 / 已用能力交叉 / 补短板 */
			var signals = [];
			var usageLower = String(ov.usageText || "").toLowerCase();
			var skillsLower = String((ov.skills || []).join(" ")).toLowerCase();
			var presetsLower = String((ov.presets || []).join(" ")).toLowerCase();
			var hay = String(plugin.name + " " + (plugin.desc || "") + " " + plugin.repo).toLowerCase();
			var kws = catKeys[plugin.category];
			if (kws && usageLower) {
				for (var k = 0; k < kws.length; k++) {
					if (usageLower.indexOf(kws[k]) !== -1) { signals.push("分类「" + plugin.category + "」命中业务词「" + kws[k] + "」"); break; }
				}
			}
			var bizKeys = ["pdf", "文档", "表格", "excel", "csv", "数据", "图片", "ocr", "视觉", "文件", "知识库", "笔记", "记忆", "obsidian", "markdown", "企业微信", "wecom", "腾讯文档", "通知", "日程", "待办", "会议", "推送", "自动化", "工作流", "流程", "巡查", "看板", "打包", "装箱", "清单", "模型", "账号", "余额", "token", "api", "成本", "费用", "终端", "bash", "shell", "cli", "git", "diff", "代码", "代理", "subagent", "团队", "协作", "team", "鞋", "工厂", "生产", "供应链", "采购", "质检"];
			for (var b = 0; b < bizKeys.length; b++) {
				if (hay.indexOf(bizKeys[b]) !== -1) { signals.push("功能命中业务词「" + bizKeys[b] + "」"); break; }
			}
			var tokens = String(plugin.name || "").toLowerCase().split(/[-_.\s/]+/).filter(function (x) { return x.length >= 3; });
			for (var t = 0; t < tokens.length; t++) {
				if (skillsLower.indexOf(tokens[t]) !== -1 || presetsLower.indexOf(tokens[t]) !== -1) { signals.push("与你已用能力「" + tokens[t] + "」相关"); break; }
			}
			var capMap = {
				"工具与能力": ["pdf", "csv", "data", "vision", "ocr", "文档", "文件"],
				"工作流与自动化": ["cfs", "vgb", "巡查", "看板", "auto", "flow"],
				"通知与集成": ["wecom", "docs", "腾讯文档", "企业微信", "通知"],
				"记忆": ["memory", "obsidian", "知识库", "记忆", "note"],
				"模型与账号接入": ["model", "余额", "token", "api", "账号"],
				"开发与运行时": ["terminal", "cli", "bash", "git", "code"]
			};
			var capKws = capMap[plugin.category];
			if (capKws) {
				var covered = false;
				for (var c = 0; c < capKws.length; c++) {
					if (skillsLower.indexOf(capKws[c]) !== -1 || presetsLower.indexOf(capKws[c]) !== -1) { covered = true; break; }
				}
				if (!covered) signals.push("补短板：你目前还没有「" + plugin.category + "」类能力");
			}
			if (signals.length > 0) {
				reasons.push("🔍 " + signals.slice(0, 3).join("；"));
			} else {
				reasons.push("与你当前业务场景关联度较低，如确实需要「" + plugin.category + "」能力可自行评估");
			}
			if (plugin.stars !== null && plugin.stars !== undefined) {
		...
		function startDrag(e, hooks) {
			...
			function onEnd() {
				if (target && target.releasePointerCapture) { try { target.releasePointerCapture(e.pointerId); } catch (err) { /* ignore */ } }
				if (target) {
		...
		function WatchPanel() {
			...
			var installedSetState = React.useState({});
			var installedSet = installedSetState[0];
			var setInstalledSet = installedSetState[1];
			...
			React.useEffect(function () {
				...
				var refreshInstalled = function () {
					callServer("/api/gh-watch/installed").then(function (j) {
						var set = {};
						((j && j.installed) || []).forEach(function (x) {
							var short = String(x.pkg || "").replace(/^@[^\/]+\//, "").toLowerCase();
							if (short) set[short] = true;
						});
						setInstalledSet(set);
					}).catch(function () { /* ignore */ });
				};
				React.useEffect(function () { refreshInstalled(); }, []);
						...
						shown.map(function (p) {
							...
							return React.createElement("div", {
								...
								React.createElement("div", { className: "hub-item-top" },
									...
									(installedSet[p.repo.toLowerCase()] || installedSet[p.name.toLowerCase()]) ? React.createElement("span", { className: "hub-badge exact", title: "已安装" }, "已安装") : null,
		...
		function InstalledPanel() {
			...
			var DESC_MAP = {
				"@deepseek-ai/dsh-subagent-acp": "外部 Agent 接入（ACP 协议）：把任务派给独立子进程里的外部 Agent（reasonix / opencode 等），后端能力、无独立界面",
				"@nanmicoder/dsh-agent-teams": "多 Agent 团队协作：船长 + 成员 + 带依赖的任务，自然语言驱动；自带 web 界面里的「团队树」监视器（仅在使用团队时出现）",
				"@liustack/modlens": "模型镜头 modlens：模型 / 账号 / 余额相关工具",
				"@deepseek-ai/dsh-base": "dsh 基础 bundle（内置核心能力）",
				"@deepseek-ai/dsh-web-app": "dsh Web 应用外壳（内置）",
				"@deepseek-ai/dsh-mcp-client": "MCP 客户端：接入 MCP 服务器（pdf / playwright / obsidian 等）"
			};
			var doCheckUpdates = function () {
				setOut("正在检查更新…");
				callServer("/api/gh-watch/updates").then(function (j) {
					var ups = (j && j.updates) || [];
					var lines = [];
					ups.forEach(function (u) {
						if (u.hasUpdate) lines.push("🔄 " + u.pkg + ": " + u.installed + " → " + u.latest);
					});
					setOut(lines.length > 0 ? lines.join("\n") : "所有插件均是最新版 ✓");
				}).catch(function (e) {
					setOut("检查更新失败：" + ((e && e.message) ? e.message : e) + "（服务端插件需重启 dsh web 后生效）");
				});
			};
			var head = null;
			var listView = null;
			...
			if (phase === "loading" && !list) {
				...
				head = React.createElement("div", { className: "hub-meta" },
					...
					React.createElement("button", {
						...
						React.createElement("button", {
							className: "hub-btn", title: "检查更新（对比 npm 最新版）", onClick: doCheckUpdates
						}, "⬆ 检查更新"));
				if (list.length === 0) {
					...
					listView = React.createElement("div", { className: "hub-list" },
						list.map(function (item) {
							...
							if (isBundle) {
								...
								stCls = "on"; stTxt = "运行中";
								rightTxt = "MCP 服务";
								subHint = "MCP 服务：配置于 cordis.patch.yml，不参与 pnpm 安装/卸载。";
							} else {
								stCls = "on"; stTxt = "已安装";
							...
							return React.createElement("div", { key: item.pkg, className: "hub-item" },
								React.createElement("div", { className: "hub-item-top" },
									React.createElement("span", { className: "hub-item-name", style: { cursor: "default" }, title: item.pkg }, item.pkg.replace(/^@[^\/]+\//, "")),
									React.createElement("span", { className: "hub-st " + stCls }, stTxt),
									React.createElement("span", { className: "hub-stars" }, rightTxt)
								),
								(DESC_MAP[item.pkg] || item.desc) ? React.createElement("div", { className: "hub-item-sub" }, "📄 " + (DESC_MAP[item.pkg] || item.desc)) : null,
		...
		function GhScanCard() {
			...
			var startResize = function (e, dir) {
				...
				function onMove(ev) {
					...
					/* 右下角为固定锚点：8 方向缩放都只改 w/h，dx/dy 保持不变 */
					var dx = oDx;
					var dy = oDy;
